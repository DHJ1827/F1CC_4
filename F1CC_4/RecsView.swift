...
struct RecsSubView: View {
        ...
        ForEach(Array(stride(from: ctr_start, to: ctr_stop, by: 11)), id: \.self) { index_v in
            ...
            HStack(alignment: .top) {
                ...
                Text(transPartString)
                    ...
                    .frame(width: 20)
                VStack {
                    if (iPartTerm == 0) {   //its ST
                        VStack {              // first 2 lines of 22 blocks of drivers and parts
                        ...
                        .border(.black)
                    } else {    // it's not ST so don't make it red font
                        VStack {              // first 2 lines of 22 blocks of drivers and parts
                            Text("resultRecs1")  // Translated(Name CL CR PL ACa NCo)
                                .font(.system(size: 9, design: .monospaced))
                                .fontWeight(.regular)
    ...
    struct RecsView: View {
        ...
        func start() {
            ...
            // default: no upgrade because PartRecID is at maxLevel
            ...
            while row_ctr < 15 {
                ...
                if (db.sDriver[RecNumber][15] == db.sDriver[RecNumber][17]) {     // CL = PL so 1 displayed line
                    ...
                    RecsDispDriver[row_ctr + 1] = RecsDispDriver[row_ctr + 1] + Spaces.prefix(7 - db.sDriver[RecNumber][21].count) + "0" + Spaces.prefix(10 - db.sDriver[RecNumber][22].count) + db.sDriver[RecNumber][22]      //PL, PR, MR, NCa, NCo
            ...
            while row_ctr < 87 {
                ...
                if (db.sPart[RecNumber][15] == db.sPart[RecNumber][17]) {     // CL = PL so 1 displayed line
                    ...
                    RecsDispParts[row_ctr + 1] = RecsDispParts[row_ctr + 1] + Spaces.prefix(7 - db.sPart[RecNumber][21].count) + "0" + Spaces.prefix(10 - db.sPart[RecNumber][22].count) + db.sPart[RecNumber][22]      //PL, PR, MR, NCa, NCo
                } else {     // CL < PL so display multiple lines
            ...
            for row_ctr in recDrRange {
                if (Recs[row_ctr][0] == Double(db.sDriver[Int(Recs[0][0])][1])) {
